<?php
/*
Plugin Name: Mero Page Builder
Plugin URI: http://tuchuk.com/
Description: A drag and drop, responsive page builder that simplifies building your website created by Greg Priday "A WordPress Expert" and hence modified by Sunil Chaulagain to suit the theme. You can donate the creater of the plugin using donate link, if you wish.
Version: 1.2.1
Author: UX-QOde
...
include plugin_dir_path(__FILE__) . 'so-widgets-bundle/so-widgets-bundle.php';
...
function siteorigin_panels_render( $post_id = false, $enqueue_css = true, $panels_data = false ) {
	...
	foreach ( $grids as $gi => $cells ) {
		...
		if( !empty( $panels_data['grids'][$gi]['style']['class'] ) ) {
			...
			$style_attributes['class'] = array('panel-row-style-default');
		}
		 $container_class='';
		...
		if( !empty($style_attributes) ) {
			...
			if(strpos($style_attributes['class'][0],'wide') == false){
			$container_class='container';
			}
					
			if(empty($style_attributes['class'])) $style_attributes['class'] = array();
			$style_attributes['class'][] = 'panel-row-style';
			if(!empty($style_attributes['id']))
			$style_attributes['class'][] = 'nav-one-page';
			...
			echo '>';
			if(strpos($style_attributes['class'][0],'overlay') !== false  || strpos($style_attributes['class'][0],'video') !== false){
				if(isset($style_attributes['overlay']) && $style_attributes['overlay']!='') {
					echo ' <div class="parallax-overlay parallax-overlay-pattern" style="'.$style_attributes['overlay'].'">';
				} else {
					echo ' <div class="parallax-overlay parallax-overlay-pattern">';
				}
			}
...
function siteorigin_panels_the_widget( $widget, $instance, $grid, $cell, $panel, $is_first, $is_last, $post_id = false ) {
	...
	$classes = array( 'panel-builder', 'widget-builder');
	...
	if(strpos($the_widget->id_base,'origin') !== false){
		...
		$the_widget->widget( array(
			'before_widget' => '<div class="well well-sm ' . esc_attr( implode( ' ', $classes ) ) . '" id="' . $id . '">',
			'after_widget' => '</div>',
