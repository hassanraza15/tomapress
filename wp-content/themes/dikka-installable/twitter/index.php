<?php
// Getting the current location and then taking out the required location of wp-load.php 
// require_once( substr(dirname(__FILE__), 0, -43).'wp-load.php' );
...
class ezTweet {
	...
	private function getTwitterJSON() {
		...
		$tmhOAuth = new tmhOAuth(array(
			...
			'consumer_key'          => esc_attr($dikka_options['twitter_consumer_key']),
			'consumer_secret'       => esc_attr($dikka_options['twitter_secret_key']),
			'user_token'            => esc_attr($dikka_options['twitter_token']),
			'user_secret'           => esc_attr($dikka_options['twitter_token_secret']),
